...
const youtubeTrendingVideosUrls = [
...
const reelVideos = [];
class JS_YoutubeTrendingShortsParser extends aParserTypes.BaseParser {
    async parse(set, results) {
        ...
        if (results.serp) {
            for (let item of reelVideos) {
                results.serp.push(item.video_id, item.title, item.description, item.channel_name, item.channel_url, item.published_time, item.tab_title, item.view_count, item.length, item.thumbnail_url, item.video_url, item.last_updated, item.proxy_geo);
            }
...
JS_YoutubeTrendingShortsParser.defaultConf = {
    version: '0.0.88',
    results: {
        flat: [
...
JS_YoutubeTrendingShortsParser.editableConf = [];

exports.JS_YoutubeTrendingShortsParser = JS_YoutubeTrendingShortsParser;
